...
class StockRegReceiptVoucherWin implements ActionListener{
    JFrame f1;
    JPanel p1;
    ...
    String colHead[]={"Voucher Number","Voucher Date","Retailer Name","Code","Locality","City","State","Receipt Mode","Transation","Transation Date","Bank Name","Amount"};
    ...
    public StockRegReceiptVoucherWin(){

        f1=new JFrame("Logistic Solution " );
        f1.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        p1=new JPanel();
        lblHead=new JLabel("RECEIPT VOUCHER",JLabel.CENTER);
    ...
    public void doConnect() {
        ...
        try {//step3
            ...
            rs=stmSelect.executeQuery("select * from   LOG_TBLRECEIPTVOUCHER where RNAME='"+rname+"'  ");

            rs.last();
            tot=rs.getRow();
            data=new Object[tot][13];
            slno=1;
            rw=0;
            rs.beforeFirst();
            while(rs.next()){
                ...
                data[rw][0]=slno;
                data[rw][1]=rs.getString("VNUM");
                data[rw][2]=rs.getString("VDATE");
                data[rw][3]=rs.getString("RNAME");
                data[rw][4]=rs.getString("CODE");
                data[rw][5]=rs.getString("LOCALITY");
                data[rw][6]=rs.getString("CITY");
                data[rw][7]=rs.getString("STATE");
                data[rw][8]=rs.getString("RMODE");
                data[rw][9]=rs.getString("TRANS");
                data[rw][10]=rs.getString("TDATE");
                data[rw][11]=rs.getString("BNAME");
                data[rw][12]=rs.getString("AMT");
...
class log_StockRegReceiptVoucherApp{
    public static void main(String []args){
        StockRegReceiptVoucherWin obj;
        obj =new  StockRegReceiptVoucherWin();
    }

...
class StockRegPurchasePNWin implements ActionListener{
    JFrame f1;
    JPanel p1;
    ...
    public void doConnect() {
        ...
        try {//step3
            ...
            while(rsCombo.next()){
                strCombo=rsCombo.getString("PURNAME");
                strCombo=strCombo.trim();
                string[i]=strCombo;
                i=i+1;
            }
            value=(String)dlg.showInputDialog(f1,"Enter  Purchase Invoice Number ","Searching",1,null,string,null);
            lblHead.setText("Issue Of Goods "+" [ "+value+" ]");
            rs=stmSelect.executeQuery("select * from   LOG_TBLPURCHASE where PURNAME='"+value+"'  ");
...
class log_StockRegPurchasePNApp{
    public static void main(String []args){
        StockRegPurchasePNWin obj;
        obj =new  StockRegPurchasePNWin();
    }

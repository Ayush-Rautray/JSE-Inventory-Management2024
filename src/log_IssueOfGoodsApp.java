...
import java.sql.*;
import java.util.*;
class IssueOfGoodsWin implements ActionListener {
    JFrame f1;
    FlowLayout flw,flw1;
    JPanel p1,p2,p3,p4;
    ...
    JLabel lblHeader,lblInvoiceNo,lblOrderNo,lblRName,lblLocality,lblCity,lblState,lblIDate,lblODate,lblPType,lblSlno,lblPCode,lblPName,lblSize,lblMRP,lblIQuant,lblAmt,lblDisPer,lblDisAmt,lblGSTper,lblGSTAmt,lblNetAmt,lbl1,lbl2,lbl3,lbl4,lbl5,lbl6,lbl7,lbl8;


    JTextField txtInvoiceNo,txtRName,txtLocality,txtCity,txtState,txtIDate,txtODate,txtSlno,txtPName,txtSize,txtMRP,txtIQuant,txtAmt,txtDisPer,txtDisAmt,txtGStper,txtGSTAmt,txtNetAmt;
    ...
    Connection conn;
    Statement stmSelect,stmComboORDNO,stmComboPCode,stmCombo,stmSave,stmDelete,stmSearching,stmUpdate,stmStockdown,stmStockNew,stmProduct1,stmGetSlno;
    ResultSet rsUser,rsComboORDNO,rsComboPCode,rsCombo,rsSearching,rsStockdown,rsProduct,rsGetSlno;
    String str,str1,str2,str3,strComboORDNO,strComboPCode,strInvoiceNo,strORDNO,strRName,strPType,strLocality,strCity,strState,strIDate,strODate,strSLNO,strPCode,strPName,strSize,strMrp,strIVQuant,strAmt,strSearching,strDisPer,strDisAmt,strGSTPer,strGSTAmt,strNetAmt,strInvoice1,strPCode1,strPCode2,strORDNO1;
    int intIVQant,intIVQant1,inttemp;


    public IssueOfGoodsWin(){
        ...
        lblHeader=new JLabel("ISSUE OF GOODS");
        ...
        txtInvoiceNo.addActionListener(this);
    ...
    public void doConnect() {
        ...
        try {//step3
            ...
            rsUser=stmSelect.executeQuery("select * from  LOG_TBLISSUEOFGOODS order by IVNUM");
    ...
    public void getComboORDNO(){
        try{
            ...
            rsComboORDNO=stmComboORDNO.executeQuery("select distinct ordno from  LOG_TBLORDERFROMRETAILER order by ordno");
            str="--select--";
            comboORDNO.addItem(str);
            while(rsComboORDNO.next()){
                strComboORDNO=rsComboORDNO.getString("ORDNO");
                strComboORDNO=strComboORDNO.trim();
                comboORDNO.addItem(strComboORDNO);
            }}catch (SQLException se){
            System.out.println("Unable to get order Data"+se);
        }
    }//ISSUE QUANTITY
    ...
    //GET INVOICE NUMBER,SLNO,ISSUE DATE;
    public void getInvoiceNo(){

        str=new java.text.SimpleDateFormat("dd-MM-yyyy").format(new java.util.Date());
        txtIDate.setText(str);
        txtIDate.setEditable(false);


        try{
            stmStockdown=conn.createStatement();
            rsStockdown=stmStockdown.executeQuery("select IVNUM,SLNO from  LOG_TBLISSUEOFGOODS order by IVNUM,SLNO");

            while(rsStockdown.next()){
                str1=rsStockdown.getString("IVNUM");
                str2=rsStockdown.getString("SLNO");

            }
            txtInvoiceNo.setText(str1);//********
    ...
    public void getSlno(){
        str1=txtInvoiceNo.getText();
        System.out.println(str1);

        try{
            stmGetSlno=conn.createStatement();
            rsGetSlno=stmGetSlno.executeQuery("select count(IVNUM) as SLNO from  LOG_TBLISSUEOFGOODS where IVNUM='"+str1+"'");
            rsGetSlno.next();

            //Float.parseFloat(txtMRP.getText())
            str=rsGetSlno.getString("SLNO");
            inttemp= Integer.parseInt(str);

            if(inttemp==0){
                inttemp=1;
                str=String.valueOf(inttemp);
                str=str.trim();
                //System.out.println(str+"New ORDNO");
                txtSlno.setText(str);
            }
            else{
                inttemp=inttemp+1;
                str=String.valueOf(inttemp);
                str=str.trim();
                //System.out.println(str+"OLD ORDNO");//geting value from data Base
                txtSlno.setText(str);
            }
        }
        catch(SQLException se){
            dlg.showMessageDialog(f1,"Error getSlno "+se,"Message",2);
        }
    }
    //ACTION LISTENER-->
    public void actionPerformed(ActionEvent ae){
        ...
        if (ae.getSource()==btnClear){
            i=0;
            str1=txtSlno.getText();
            i=Integer.parseInt(str1);
            i=i+1;
            str1=String.valueOf(i);
            txtSlno.setText(str1);
            //txtSlno.setEditable(false);
            i=0;
        ...
        if (ae.getSource()==btnSave){
            ...
            if(strPType.isEmpty()){
                dlg.showMessageDialog(f1," Data incomplete ","Message",2);
                return;
            ...
            try{
                ...
                intIVQant=Integer.parseInt(strIVQuant);
                stmStockdown=conn.createStatement();
                stmStockdown.executeUpdate("update LOG_TBLPRODUCT set OPSTK=OPSTK-'"+intIVQant+"' where PCODE='"+strPCode+"'");


                //SAVING-->
                stmSave= conn.createStatement();
                stmSave.executeUpdate("insert into LOG_TBLISSUEOFGOODS values ('"+strInvoiceNo+"','"+strIDate+"','"+strORDNO+"','"+strODate+"','"+strRName+"','"+strLocality+"','"+strCity+"','"+strState+"','"+strPType+"','"+strSLNO+"','"+strPCode+"','"+strPName+"','"+strSize+"','"+strMrp+"','"+strIVQuant+"','"+strAmt+"','"+strDisPer+"','"+strDisAmt+"','"+strGSTPer+"','"+strGSTAmt+"','"+strNetAmt+"')");
        ...
        //LAST-->
            ...
            try{
                ...
                if(option==0){
                    ...
                    intIVQant=Integer.parseInt(strIVQuant);
                    stmStockdown=conn.createStatement();
                    stmStockdown.executeUpdate("update LOG_TBLPRODUCT set OPSTK=OPSTK+'"+intIVQant+"' where PCODE='"+strPCode1+"'");
                    //RECORD DELETE-->
                    stmDelete=conn.createStatement();
                    stmDelete.executeUpdate("delete from  LOG_TBLISSUEOFGOODS where IVNUM='"+strInvoice1+"'and PCODE='"+strPCode1+"'");
        ...
        //SELECT-->
            ...
            try{
                ...
                rsSearching=stmSearching.executeQuery("select * from LOG_TBLISSUEOFGOODS where IVNUM='"+strSearching+"' and  PCODE='"+strPCode2+"'"  );
        ...
        //UPDATE-->
            ...
            try {
                ...
                if(option==0){
                    ...
                    if (intIVQant1!=intIVQant) {
                        stmStockdown=conn.createStatement();
                        stmStockdown.executeUpdate("update LOG_TBLPRODUCT set OPSTK=OPSTK+'"+intIVQant1+"'  where PCODE='"+strPCode1+"'");
                        stmStockdown.executeUpdate("update LOG_TBLPRODUCT set OPSTK=OPSTK-'"+intIVQant+"'  where  PCODE='"+strPCode1+"'");

                    }


                    stmUpdate.executeUpdate("update LOG_TBLISSUEOFGOODS set IVDATE ='"+strIDate+"', ORDNO='"+strORDNO+"',LOCALITY='"+strLocality+"', CITY='"+strCity+"', STATE='"+strState+"',PTYPE='"+strPType+"',SLNO='"+strSLNO+"',PCODE='"+strPCode+"',PNAME='"+strPName+"',PSIZE='"+strSize+"',MRP='"+strMrp+"',IVQUANT='"+strIVQuant+"',AMT='"+strAmt+"',DISPER='"+strDisPer+"',DISAMT='"+strDisAmt+"',GSTPER='"+strGSTPer+"',GSTAMT='"+strGSTAmt+"',NETAMT='"+strNetAmt+"' where IVNUM = '"+strInvoice1+"'and PCODE='"+strPCode1+"'");
            ...
            try{
                ...
                rsCombo= stmCombo.executeQuery("Select * from LOG_TBLORDERFROMRETAILER  where ORDNO='" + str + "'");
                if(rsCombo.next()){
                    str1=rsCombo.getString("RNAME");
                    str1=str1.trim();
                    txtRName.setText(str1);
                    ...
                    str3=rsCombo.getString("ODATE");
                    str3=str3.trim();
                    txtODate.setText(str3);
        ...
        else if(ae.getSource()==txtInvoiceNo){
            getSlno();
        }
